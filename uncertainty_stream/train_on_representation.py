...
class Trainer:
    def __init__(self, config):
        ...
        # Initialize in_representation model
        print(f' Model: {self.config.ors_architecture} '.center(self.terminal_width, '*'))
        model_type = import_module('models.' + self.config.ors_architecture)
        ...
        self.optimizer = torch.optim.RMSprop(self.model.parameters(), lr=self.config.ors_learning_rate, alpha=0.95)
        ...
        self.model_checkpoint = ModelCheckpoint(weight_dir='./weights/on_representation/')
        if config.resume_training:
            ...
            logs = glob.glob('./logs/on_representation/.*') + glob.glob('./logs/on_representation/*')
            for f in logs:
                os.remove(f)

        # logging
        self.writer = SummaryWriter('logs/on_representation/')
