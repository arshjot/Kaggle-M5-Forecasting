...
class Encoder(nn.Module):
    def __init__(self, input_size, cal_embedding_sizes, config):
        ...
        self.fc = nn.Linear(self.input_size, 256)
        self.fc_time = nn.Linear(self.seq_length, 28)
        self.rnn = nn.LSTM(256, config.rnn_num_hidden,
                           config.rnn_num_layers, dropout=config.enc_rnn_dropout, bidirectional=config.bidirectional)

    def forward(self, x, x_cal_emb):
        x, x_cal_emb = x.permute(1, 0, 2), x_cal_emb.permute(1, 0, 2)  # make time-major
        ...
        x_rnn = torch.cat([x, output_emb_cal], 2).permute(1, 0, 2)

        x_rnn = self.fc(x_rnn).permute(0, 2, 1)
        x_rnn = self.fc_time(x_rnn).permute(2, 0, 1)
...
class Decoder(nn.Module):
    def __init__(self, input_size, cal_embedding_sizes, output_size, config):
    ...
    def forward(self, x, x_cal_emb, hidden):
        x, x_cal_emb = x.permute(1, 0, 2), x_cal_emb.permute(1, 0, 2)  # make time-major
        ...
        x_rnn = self.fc(x_rnn).permute(1, 0, 2)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, x_enc, x_cal_enc_emb, x_dec, x_cal_dec_emb, x_prev_day_sales_dec):
        ...
        encoder_output, hidden = self.encoder(x_enc, x_cal_enc_emb)
        ...
        for timestep in range(0, pred_len):
            ...
            decoder_output, hidden = self.decoder(dec_input, x_cal_dec_emb[:, timestep, :].unsqueeze(1), hidden)
...
def create_model(config):
    ...
    num_features_enc = 12 + config.graph_emb_dim + cal_embedding_sizes[1] * 2
    num_features_dec = 12 + config.graph_emb_dim + cal_embedding_sizes[1] * 2
    enc = Encoder(num_features_enc, cal_embedding_sizes, config)
    dec = Decoder(num_features_dec, cal_embedding_sizes, 9, config)
