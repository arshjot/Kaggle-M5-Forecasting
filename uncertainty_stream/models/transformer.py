...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_cal_emb):
        ...
        x = torch.cat([x, output_emb_cal], 2).permute(1, 0, 2)
...
class Decoder(nn.Module):
    ...
    def forward(self, enc_out, y_mask, x, x_cal_emb):
        x, x_cal_emb = x.permute(1, 0, 2), x_cal_emb.permute(1, 0, 2)  # make time-major
...
class TransformerModel(nn.Module):
    ...
    def forward(self, x_enc, x_cal_enc_emb, x_dec, x_cal_dec_emb, x_prev_day_sales_dec):
        ...
        encoder_output = self.encoder(x_enc, x_cal_enc_emb)
        ...
        if self.training:
            ...
            decoder_output = self.decoder(encoder_output, y_mask, x_dec, x_cal_dec_emb)
